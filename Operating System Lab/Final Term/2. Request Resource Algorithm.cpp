...
/*
#At first find Need(P1) = Max - Allocation (P1)
#Steps:
1. If Request(i) <= Need(i), goto 2
2. If Request(i) <= Available(i), goto 3
3. (i) Available = Available - Request(i)
   (ii) Allocation = Allocation + Request(i)
   (iii) Need(i) = Need(i) - Request(i)
4. Check if new state is safe or not using Banker's Algorithm
#The New table will be updated:
(i) P(1)
(ii) Available
(iii) Need(1)
5. Banker's Algorithm Safe Sequence
Input:
1
1 0 2
Output:
Which process requests the resources?Enter the three resources A, B, and C:3 0 2 
2 3 0 
0 2 0 
